...
matrix_float3x3 AAPL_SIMD_OVERLOAD matrix3x3_from_quaternion(quaternion_float q) {
    float xx = q.x * q.x;   // x^2
    float xy = q.x * q.y;
    float xz = q.x * q.z;
    float xw = q.x * q.w;
    float yy = q.y * q.y;   // y^2
    float yz = q.y * q.z;
    float yw = q.y * q.w;
    float zz = q.z * q.z;   // z^2
    float zw = q.z * q.w;
...
// Transpose the matrix follow by an inverse.
...
quaternion_float AAPL_SIMD_OVERLOAD quaternion_from_matrix3x3(matrix_float3x3 m)
{
    ...
    // The sum of the elements of the main diagonal plus 1
...
vector_float3 AAPL_SIMD_OVERLOAD right_direction_vector_from_quaternion(quaternion_float q) {
    ...
    direction = vector_normalize(direction);
    // Negate for a right-handed coordinate system
    // Caller must do it manually e.g.
    // vector_float3 v = -right_direction_vector_from_quaternion(q);
    return direction;
}
